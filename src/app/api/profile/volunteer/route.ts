...
export async function POST(req: Request) {
  try {
    ...
    const { name: newName, ...profileFields } = body;

    // Parse and validate the request body (exclude name from profile validation)
    const validatedData = volunteerProfileSchema.parse(profileFields);

    // Ensure the user has a role of VOLUNTEER
